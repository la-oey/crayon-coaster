...
#write new full sim data
write_csv(df, "fullsim.csv")
```
...
```{r}
# unique trials and success, ordered by probability
succdf %>%
  filter(level == "basic_far" & wind == "none") %>%
  arrange(pSucc)

succdf %>%
  filter(level == "boomerang_middle" & wind == "right") %>%
  arrange(pSucc)
```
