...
def get_lvlwind(text):
  end = "/connect-"
  end_idx = text.find(end)
  if end_idx == -1:
    return None  # Return None if the end is not found
  return text[:end_idx]
...
def get_img(text):
  ...
  start_idx += len(start)
  return "connect-" + text[start_idx:]


def find_index_by_key(dicts, image):
...
outputfile = open("output_loo_" + ratedim + "_missed8.csv", "w")
...
missing = json.load(open("../analysis/LLM/missing_prompt_"+sys.argv[1]+"_llmratings8.json"))
#re-ran feasible3 because of repeats
...
for m in missing:
  ...
  cloned = humanratings[key].copy()
  selected = cloned[find_index_by_key(cloned, img)]
